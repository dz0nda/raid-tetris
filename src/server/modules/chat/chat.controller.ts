...
import { sendChatMessageSchema } from './chat.schema';
...
export class ChatsController {
  ...
  constructor(service: ChatService, socket: SocketService) {
  ...
  public sendChatMessage(req: Request<{ text: string }>) {
    ...
    const messages = this.service.sendMessage(socket.getRoom, socket.getUsername, req.data.text);
